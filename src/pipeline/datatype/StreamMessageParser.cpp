...
static std::tuple<DatatypeEnum, size_t, size_t> parseHeader(streamPacketDesc_t* const packet) {
    ...
    const auto info = fmt::format(", total size {}, type {}, metadata size {}", packet->length, (int)objectType, serializedObjectSize);
...
std::shared_ptr<RawBuffer> StreamMessageParser::parseMessage(streamPacketDesc_t* const packet) {
    ...
    throw std::runtime_error(
        fmt::format("Bad packet, couldn't parse, total size {}, type {}, metadata size {}", packet->length, (int)objectType, serializedObjectSize));
}
...
std::shared_ptr<ADatatype> StreamMessageParser::parseMessageToADatatype(streamPacketDesc_t* const packet, DatatypeEnum& objectType) {
    ...
    throw std::runtime_error(fmt::format(
        "Bad packet, couldn't parse (invalid message type), total size {}, type {}, metadata size {}", packet->length, (int)objectType, serializedObjectSize));
}
